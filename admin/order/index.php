...
<?php
	...
	foreach($data as $item) {
		echo '<tr>
					...
					<td style="width: 50px">';
		if($item['status'] == 0) {
			echo '<button onclick="changeStatus('.$item['id'].', 1)" class="btn btn-sm btn-success" style="margin-bottom: 10px;">Duyệt</button>
			<button onclick="changeStatus('.$item['id'].', 2)" class="btn btn-sm btn-danger">Hủy</button>';
		} else if($item['status'] == 1) {
			echo '<label class="badge badge-success">Duyệt</label>';
		} else {
